...
static void wrctl(DisasContext *dc, uint32_t code, uint32_t flags)
{
    ...
    if (IS_USER(dc)) {
...
#if !defined(CONFIG_USER_ONLY)
            gen_helper_cr_ienable_write(dc->cpu_env, load_gpr(dc, instr.a));
        ...
        case CR_STATUS:
            /* Re-evaluate interrupts */
#if !defined(CONFIG_USER_ONLY)
            gen_helper_cr_status_write(dc->cpu_env, load_gpr(dc, instr.a));
            tcg_gen_movi_tl(dc->cpu_R[R_PC], dc->pc + 4);
            dc->is_jmp = DISAS_UPDATE;
#endif
            break;
...
void gen_intermediate_code(CPUState *cs, TranslationBlock *tb)
{
    ...
    if (qemu_loglevel_mask(CPU_LOG_TB_IN_ASM)
        ...
        log_target_disas(cs, tb->pc, dc->pc - pc_start, 0);
        qemu_log("\n");
        qemu_log_unlock();
    }
#endif
